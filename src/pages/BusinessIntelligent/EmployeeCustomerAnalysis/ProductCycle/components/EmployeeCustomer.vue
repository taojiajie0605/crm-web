<template>
    <div class="container">
        ...
        <Chart class="chart" ref="ProductCycleEmployeeCustomerChart" />
        <TotalTable
            class="table"
            :columns="columns"
            :data="data"
...
import Chart from './chart/EmployeeCustomerChart';
...
export default {
    ...
    data() {
        return {
            columns: [
                { title: '姓名', dataIndex: 'employeeName', align: 'center' },
                {
                    title: '成交周期（天）',
    ...
    methods: {
        ...
        getTableData(param) {
            // 初始化数据
            request('/ProductCycleEmployeeTableList', METHOD.POST, {
                ...param,
            }).then((data) => {
                this.data = data.data;
            });
        ...
        getChartData(param) {
            this.$refs.ProductCycleEmployeeCustomerChart.getChartData(param);
        },
    },
    mounted() {
        this.search({});
