...
export default {
    ...
    methods: {
        ...
        getTableData(param) {
            // 初始化数据
            request('/CustomerSourceChart', METHOD.POST, {
                ...param,
            }).then((data) => {
                // 组装表格
                this.columns = [
                    { title: '客户来源', dataIndex: 'type', align: 'center' },
                ];
                ...
                for (let i = 0; i < data.data.length; i++) {
                    ...
                    this.columns.push({
                        title: item.source,
                        dataIndex: item.source,
                        align: 'center',
                    });
                    totalCustomer[item.source] = item.totalCustomer;
                    dealCustomer[item.source] = item.dealCustomer;
                }
                this.data = [];
