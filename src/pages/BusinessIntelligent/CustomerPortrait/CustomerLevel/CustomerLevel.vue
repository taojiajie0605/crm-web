...
export default {
    ...
    methods: {
        ...
        getTableData(param) {
            // 初始化数据
            request('/CustomerLevelChart', METHOD.POST, {
                ...param,
            }).then((data) => {
                // 组装表格
                this.columns = [
                    { title: '客户级别', dataIndex: 'type', align: 'center' },
                ];
                ...
                for (let i = 0; i < data.data.length; i++) {
                    ...
                    totalCustomer[item.level] = item.totalCustomer;
                    dealCustomer[item.level] = item.dealCustomer;
                }
                this.data = [];
