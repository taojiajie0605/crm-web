    ...
    <a-card>
        ...
        >
            ...
            <div slot="operation" slot-scope="record">
                <div v-if="record.station === 1">
                    <a-button type="primary" @click="passorreject(record, 1)">
                        通过
                    </a-button>
                    <a-button
                        type="primary"
                        style="margin-left: 10px;"
                        @click="passorreject(record, 2)"
                    >
                        拒绝
                    </a-button>
                </div>
...
export default {
    ...
    methods: {
        ...
        fetch(params = {}) {
            ...
            this.loading = true;
            request('/myAttleave', METHOD.POST, {
                ...params,
            }).then((data) => {
        ...
        //通过与拒绝
        passorreject(record, type) {
            const _this = this;
            this.$confirm({
                title: '提示',
                content: <div style="color:black;">您确定此操作吗?</div>,
                onOk() {
                    request('/passorreject', METHOD.POST, {
                        record: record,
                        type: type,
                    }).then((data) => {
                        _this.refresh();
